export default {
    ...
    handleTouchMove(event) {
      if (this.swipeStart) {
        ...
        if (elapsedTime <= this.mdSwipeTime) {
          if (
            Math.abs(actualX) >= this.mdSwipeThreshold &&
            Math.abs(actualY) <= this.mdSwipeRestraint
          ) {
            this.swiped = actualX < 0 ? "left" : "right";
          } else if (
            Math.abs(actualY) >= this.mdSwipeThreshold &&
            Math.abs(actualX) <= this.mdSwipeRestraint
          ) {
  ...
  mounted() {
    if (this.mdSwipeable) {
      this.getSwipeElement.addEventListener(
        "touchstart",
  ...
  beforeDestroy() {
    if (this.mdSwipeable) {
      this.getSwipeElement.removeEventListener(
        "touchstart",
        this.handleTouchStart,
        false
      );
