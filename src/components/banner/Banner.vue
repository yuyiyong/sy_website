<!--
 * @LastEditors:  
 * @LastEditTime: 2021-06-08 15:37:01
 * @FilePath: /sy_website/src/components/banner/Banner.vue
-->
<!-- slider 组件 for VUE 2.x -->
<template>
  <div
    class="vue-only-slider"
    :class="{
      'slider-left': touchOffsetX < 0,
      'slider-right': touchOffsetX > 0,
      reseting: reseting,
    }"
    ref="sliderWrap"
  >
    <!-- :style="{ width: `${itemWidth}px`, height: `${sliderHeight}px` }" -->
    <div
      class="slider-wrap"
      :style="{ width: `${itemWidth}px`, height: `${sliderHeight}px` }"
    >
    ...
    <div class="pagination" v-if="pagination">
      <i
        ...
        :class="{ on: current - loopPushNum === index }"
        :key="index"
        >{{ item }}</i
      >
    </div>
...
module.exports = {
  ...
  computed: {
    ...
    sliderHeight() {
      return parseFloat(
        this.itemWidth * (this.sourceHeight / this.sourceWidth)
      );
    },
    ...
    _items() {
      if (this.loop && this.items.length > 1) {
        return this.items
          .slice(this.items.length - this.loopPushNum)
          .concat(this.items, this.items.slice(0, this.loopPushNum));
      } else {
        return this.items;
      }
    },
  ...
  methods: {
      ...
      if (init) {
        ...
        let inDom =
          ...
          up = this.anims.replace(/{[^}]+}/g, (val) => {
            ...
            return (
              v.min +
              (v.max - v.min) * Math.abs(this.touchOffsetX / this.itemWidth)
            );
          }),
    ...
    resetPosition() {
      ...
      this.timers = setTimeout(() => {
        ...
        if (this.current === 1 || this.current === this.itemsLength - 2) {
          this.current =
            this.current === 1
              ? this.itemsLength - (this.loopPushNum + 1)
              : this.loopPushNum;
    ...
    removeListener() {
      this.sliedrWrap.removeListener("touchstart", this.startHandler, false);
      this.sliedrWrap.removeListener("touchmove", this.moveHandler, false);
      this.sliedrWrap.removeListener("touchend", this.endHandler, false);
    },
  },
};
</script>
